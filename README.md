...
AI 시대 개발자의 가치는 두 가지라고 생각합니다. **일 AI에게 잘 시키는 것**, 그리고 **AI가 만든 결과물을 잘 리뷰하는 것**.
...
예를 들어 [ios-workflow](deploy/skills/ios-workflow/README.md)는 제가 iOS 앱을 만드는 방식을 그대로 프롬프트로 옮긴 것입니다. 기획서나 디자인 시안을 넣으면 PR이 나옵니다. 피그마도 그런 입력 자료의 하나일 뿐이라, 별도 연동 도구 없이 사용자가 준 URL이나 캡처를 그대로 읽습니다.
