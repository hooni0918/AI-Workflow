...
작업은 여러 단계로 나뉘어 각각 독립된 맥락에서 진행됩니다. 자료를 읽고 무엇을 왜 해야 하는지 정리한 뒤 할 일을 PR 단위로 쪼개고, 화면을 UIKit으로 마크업하는 작업을 PR 작업과 동시에 만들고, PR마다 설계와 구현, 본문 작성을 거칩니다. 설계 단계에서는 Clean Architecture 레이어(Feature·Domain·Data 등)의 경계를 먼저 긋고, 새 모듈이나 의존성이 필요하면 SPM 패키지 구성을 함께 정합니다.
...
- **동시성·관찰 모델.** Swift 6의 strict concurrency가 컴파일러 강제로 걸립니다(`SWIFT_STRICT_CONCURRENCY=complete`). `@Observable` 매크로는 iOS 17+ 한정으로 배포 타깃(iOS 17)에서 가용하나, UI가 UIKit이라 쓰지 않습니다.
