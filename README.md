...
예를 들어 [ios-workflow](deploy/skills/ios-workflow/README.md)는 제가 iOS 앱을 만드는 방식 자체를 프롬프트로 옮긴 것입니다. 기획서나 디자인 시안 같은 입력 자료를 넣으면 PR이 나옵니다 (피그마는 그 입력 자료의 한 예시일 뿐, 별도 figma 연동 도구에 의존하지 않습니다 — 사용자가 제공한 URL·캡처를 자료로 처리합니다).

작업은 여러 단계로 나뉘어 각각 독립된 맥락에서 진행됩니다. 자료를 읽고 무엇을 왜 해야 하는지 정리한 뒤 할 일을 PR 단위로 쪼개고, 화면을 SwiftUI로 마크업하는 작업을 PR 작업과 동시에 만들고, PR마다 설계와 구현, 본문 작성을 거칩니다. 설계 단계에서는 화면(View)과 상태(Model), 화면 전환(Coordinator)의 경계를 먼저 긋고, 새 화면이나 의존성이 필요하면 Tuist 모듈 구성을 함께 정합니다.
...
- **타입 시그니처는 컴파일러가 보증합니다.** stub(빈 구현)을 먼저 깔고 시그니처 정합만 맞춰 두면 호출부와의 타입 어긋남은 컴파일이 막아 줍니다. 하지만 미구현 누락(본문이 비어 있는 함수)은 컴파일러가 잡지 못합니다. 특히 저장 프로퍼티나 `@Published` 초기값처럼 값이 반드시 있어야 하는 자리는 `fatalError`로 메울 수 없으므로, 빈 채로 두면 컴파일은 통과해도 런타임에서야 드러납니다.
- **레이어 경계는 Tuist로 검사합니다.** `tuist inspect`는 "선언하지 않은 import"를 잡고, 모듈 사이의 역방향 의존(단방향이어야 할 의존이 뒤집힌 경우)은 `tuist graph`로 의존 그래프를 그려 해석합니다. 둘은 잡는 대상이 다르므로 구분해서 씁니다.
- **동시성·관찰 모델은 도입 환경을 확인하고 씁니다.** Swift 6의 strict concurrency가 컴파일러 강제로 걸리는 것은 해당 모듈이 strict mode일 때뿐입니다(가정: 모듈별 설정). `@Observable` 매크로는 iOS 17+에서만 쓸 수 있으므로, 배포 타깃이 그 아래면 이 보증을 받을 수 없습니다.

### 테스트로 거르기

AI가 코드를 만들거나 고칠 때마다, 기존 동작이 깨지지 않았는지 테스트가 자동으로 확인합니다. 사람이 직접 검증해야 할 범위가 그만큼 줄어듭니다.

빌드와 테스트는 `tuist xcodebuild build`와 `tuist test`로 돌립니다(가정: Tuist 4.x). `tuist build`는 4.x에서 deprecated이므로 쓰지 않습니다.
...
자동 검사가 잡지 못하는 문제(화면과 데이터 로직이 섞여 분리가 필요한 경우, Coordinator를 거치지 않은 화면 전환 등)는 AI 리뷰가 맡습니다. 반복된 지적은 [coding-standards](deploy/contexts/coding-standards/README.md)에 규칙으로 쌓아 두고 [code-review](deploy/skills/code-review/README.md) 스킬이 그 규칙을 읽어 대조하므로, 사람이 리뷰에 쓰는 시간이 점점 줄어듭니다.

## 배포

스킬과 규칙, hook은 `make`로 배포합니다. 배포 인프라는 Python(`python3 scripts/*.py`)으로 구현되어 있고, `Makefile`이 그 진입점입니다.
...
- [ios-workflow](deploy/skills/ios-workflow/README.md): 기획서나 디자인 시안 같은 입력 자료를 넣으면 PR이 나오는 단계별 프로세스
  - 요구사항 리뷰(step-1.2): 자료를 구현 전에 iOS 개발자 관점에서 점검 — 화면 종류별 체크리스트는 [requirement-review/screen-type](deploy/skills/ios-workflow/requirement-review/screen-type/)

### 모듈 구성

- [tuist-module](deploy/skills/tuist-module/README.md): 새 화면·기능에 맞춰 Tuist 모듈을 추가하고 의존 그래프를 단방향으로 유지
