...
- 모듈 내부 헬퍼(권장).
- 시각 마크업(프로젝트 UI 프레임워크의 화면 코드)은 stub 대상이 아니다 — MARKUP 세션의 완성본을 그대로 가져오므로.
...
- **상태 패턴·관찰 모델**: 미정으로 둔다(전제는 assumptions.md 「Observation·동시성 전제」). 어떤 상태/관찰 패턴을 쓰는지는 프로젝트 프로필/architecture.md가 1차 출처. 없으면 사용자에게 묻거나 안전 가정(가정 라벨).
