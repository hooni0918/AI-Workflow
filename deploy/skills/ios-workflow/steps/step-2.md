...
- **인프라성 PR → 순차**: SPM 의존성 추가·모듈 셋업 등은 stub 시점 병렬 효과가 작으므로 순차 (이전 PR 머지 후 다음 PR_{N+1}_PLAN spawn). 단 "인프라성이라 stub 불필요"는 디폴트가 아니다 — 의존성·설정·테스트 TODO가 코드로 표현 가능하면 stub 대상 (step-4 참조)
