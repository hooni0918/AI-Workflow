...
- **stub가 보증하는 것 / 못 하는 것**: 시그니처 정합만 컴파일러가 보증하고, 미구현 누락(빈 함수)은 컴파일러가 못 잡는다. 빈 함수는 테스트 오라클·`it.todo` 커버리지가 잡는다.
...
1. **LLM이 PR 작업 분석** — 둘 중 하나라도 해당하면 코드로:
   ...
   - **조건 2 (코드로 표현 가능한 모든 계획)**: 시그니처가 없어도 파일로 표현 가능한 계획은 전부 코드/stub로 — 의존성(SPM `Package.swift` 추가), 설정, 테스트 의도(`it.todo`). 이들을 어떤 md에도 산문으로 나열하지 않는다.
