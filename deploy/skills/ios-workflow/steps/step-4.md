...
대상 컨벤션 1차 소스: `*ARCHITECTURE*`, `docs/conventions/*`, `Project.swift`/`Workspace.swift`(모듈·의존 정의), 기존 모듈의 평행 사례, 작업 대상 디렉터리 조상 체인 `CLAUDE.md`. `conventions-index.md`가 있으면 거기 등재 경로를 grep 출발점으로 우선한다.

사유: 컨벤션 위반은 reviewer가 잡기 전 PLAN 단계에서 막아야 한다. 레이어 역방향 의존은 [conventions/tuist.md](../conventions/tuist.md) 「tuist graph」가 사후에 잡지만, 사전 grep으로 차단하는 게 비용이 낮다.
...
| `implementation.md` | `pr{N}/persistent/` | 구현 순서·커밋 분할·회귀 체크리스트·`it.todo` 매핑 | 대부분 작성됨 |
...
- **stub가 보증하는 것 / 못 하는 것**: 시그니처 정합만 컴파일러가 보증하고, 미구현 누락(빈 함수)은 컴파일러가 못 잡는다. 빈 함수는 `tuist test` 오라클·`it.todo` 커버리지가 잡는다.
- **저장 프로퍼티·`@Published` 초기값은 fatalError로 못 메운다**: 함수 본문은 `fatalError("not implemented")`로 미루지만, 저장 프로퍼티/`@Published` 초기값은 의미 있는 디폴트 또는 placeholder + `// TODO [AI_IMPL]:`로 박아야 컴파일된다.
...
1. **LLM이 PR 작업 분석** — 둘 중 하나라도 해당하면 코드로:
   ...
   - **조건 2 (코드로 표현 가능한 모든 계획)**: 시그니처가 없어도 파일로 표현 가능한 계획은 전부 코드/stub로 — 의존성(SPM/Tuist `Package.swift`·`Project.swift` 추가), 설정, 테스트 의도(`it.todo`). 이들을 어떤 md에도 산문으로 나열하지 않는다.
...
**"인프라성(의존성·모듈 셋업) PR이라 stub 불필요"는 잘못된 디폴트다** — deps·설정·`it.todo`가 코드로 표현 가능하므로 조건 2에 의해 stub 대상이다.
...
- stub 커밋이 빌드·SwiftLint를 통과하는지 확인 후 커밋한다: `tuist xcodebuild build`, `swiftlint lint --strict` ([conventions/tuist.md](../conventions/tuist.md))

#### [CRITICAL] 포맷팅·SwiftLint 영역 한정

전체 자동 수정(`swiftlint --fix` 인자 없이 전역) 금지. 본 PR 영역만 한정 적용 ([conventions/tuist.md](../conventions/tuist.md) 「영역 한정」). 사유: 전역 자동 수정은 PR 외 파일을 일괄 변경 → 글로벌 룰 「내 작업 외 변경은 커밋하지 않는다」 위반.
...
리뷰 체크리스트:
- **컨벤션 대조**: 각 산출물 내용 기반으로 관련 컨벤션을 찾아 대조. `reference.md`에 컨벤션 경로가 누적 명시되어 있는지 확인. stub 파일이 빌드·SwiftLint를 통과하는지 확인 (코드라서 가능). stub 작성 룰 준수 ([conventions/stub.md](../conventions/stub.md)) — 저장 프로퍼티 초기값·`@Observable`/`ObservableObject` 분기·throw 패턴 직접 점검.
- **코드-narrative 오배치 검출**: 모든 md에 코드로 표현 가능한 내용(deps·설정·`it.todo`·시그니처)이 산문으로 들어가 있지 않은지. 있으면 stub 코드로 옮기도록 지적.
- **설계 타당성 역추적**: decisions.md 기술 결정·overview.md 의도를 기준으로 파생 산출물이 충실히 반영하는지.
- **산출물 간 정합성**: stub View의 props 타입을 ViewModel stub이 일관 소비하는지. 모든 `it.todo`가 implementation.md의 어느 커밋에서 다뤄지는지 대조. 테스트는 구현 커밋에 함께 포함.
- **자유 리뷰**.

### 2. 종료 게이트 (`it.todo` 매칭)

산출물 리뷰와 별개로 직접 수행. implementation.md의 커밋 계획과 stub의 `it.todo`를 매칭/누락/면제로 분류. stub 없는 PR이면 `it.todo` 0건 → 면제로 분류하고 사유 명시.
