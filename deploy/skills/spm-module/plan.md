...
- 프로젝트에 `.claude/docs/clean-architecture.md`가 있으면 그 문서가 레이어 목록·경계의 **1차 출처(single source)**다 — 거기 정의된 레이어·허용 방향을 따른다. 이 step은 방법론(레이어 단방향·수평 금지·사이클 금지)을 담고, 프로젝트 고유의 정확한 경계는 그 문서에 위임한다.
...
- **분리**: 한 모듈을 둘 이상으로 쪼갠다 (예: 비대한 Feature에서 순수 도메인 규칙을 Domain으로, 공용 기반을 Core로).
- **병합**: 둘 이상을 합친다.
- **의존 재배치**: 모듈은 그대로 두고 의존 방향·경로만 바꾼다 (예: Feature 간 직접 의존을 Domain 경유로 교정).

## 2. 레이어 배치 판정

각 대상 모듈을 어느 레이어에 둘지 판정한다. 기준은 [conventions/module-layers.md](conventions/module-layers.md)의 레이어 정의·책임이며, 프로젝트에 `.claude/docs/clean-architecture.md`가 있으면 그 문서의 레이어 목록을 1차 출처로 우선한다.

판정 절차:

1. 모듈이 **하는 일**을 한 줄로 적는다.
2. 레이어 책임과 대조해 배치한다.
   - 화면·사용자 흐름·UIKit ViewController → Feature
   - 화면과 무관한 도메인 규칙·엔티티·값객체·UseCase/Repository 프로토콜 → Domain
   - Repository 구현·원격 통신·DTO 매핑 → Data
   - 네트워킹 추상·전송 계층 → Networking
   - 디자인 토큰·공용 UI 컴포넌트 → DesignSystem
   - 레이어 무관 공용 기반(확장·유틸·로깅 등) → Core
3. 배치가 애매하면 **더 낮은(더 의존받는) 레이어로 내리는 쪽**을 우선 검토한다. 상위 레이어는 갈아끼우기 쉽지만, 하위로 내린 것을 다시 올리려면 의존자가 깨진다.

## 3. 의존 방향 설계

각 모듈이 **무엇을 의존할지**를 방향과 함께 적는다. 허용 방향의 단일 출처는 [conventions/module-layers.md](conventions/module-layers.md)(프로젝트 `.claude/docs/clean-architecture.md`가 있으면 그것이 우선)다.
...
- **App.xcodeproj 링크 계획**: 새 모듈이 앱에서 직접 쓰여야 하면, App 타깃이 그 로컬 패키지 라이브러리를 링크하도록 하는 변경을 설계서에 적는다 (App은 컴포지션 루트로서 전 모듈을 의존). App 타깃에 직접 링크할지, 상위 패키지를 경유해 transitive로 끌어올지 방향 규칙에 맞춰 정한다 — 반영 절차는 APPLY가 수행한다.
...
- 모듈별 레이어 배치와 근거 (2단계)
- 의존 방향 텍스트 그래프 (3단계)
- `Package.swift` 변경안 diff + App.xcodeproj 링크 계획 (4단계)
- **가정 라벨**: 대상 모듈의 테스트 러너(`swift test` vs `xcodebuild test`)·UDID, VC 상태 패턴(전용 패턴 미정) 등 미확인 항목 (SKILL.md 「미확정은 "가정" 라벨」). iOS 17·Swift 6·strict concurrency는 빌드설정으로 확정된 사실이므로 가정으로 적지 않는다.
...
- 설계서·`Package.swift` 변경안이 APPLY의 입력임을 명시한다. 단, APPLY의 **검증 기준은 설계서가 아니라** [conventions/module-layers.md](conventions/module-layers.md)와 각 `Package.swift`의 `dependencies` 배열·빌드 출력임을 함께 안내한다 (자기증명 차단).
