...
`body`가 길다고 기계적으로 잘라 하위 `View`로 빼면, 부모와 자식이 같은 상태를 잘게 나눠 들고 있어 오히려 추적이 어려워진다. 자른 단위가 **하나의 의미(한 구역, 한 책임)**를 이루는지 본다. 의미 단위가 아니면 자르지 말고, 자를 거면 그 단위가 독립된 입력만으로 그려지도록 추상화한다. `View` 분리·상태 전달의 1차 출처는 네 프로젝트의 `.claude/docs`다.
