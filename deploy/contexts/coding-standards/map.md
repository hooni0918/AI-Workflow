...
- 범용 규칙·원칙(`rules/`·`principles/`): 어떤 iOS 프로젝트에든 적용
- 스택별 예시(`examples/`): 직접 적용하지 않고, 프로젝트가 자기 아키텍처·UI·네이밍으로 대체

`rules/`·`principles/`는 스택을 고정하지 않는 범용 기준이고, `examples/`는 특정 스택에서 그 기준을 구체화한 예시다(마스터 표준이 아니라 프로젝트가 대체).
