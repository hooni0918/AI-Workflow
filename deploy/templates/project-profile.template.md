...
<!-- 채우는 법: 사용자 수동 동작 테스트에 앞서 시나리오를 자동 실행할 스킬을 적는다. 소비자가 이름으로 소환하므로 스킬이 아닌 러너(테스트 스킴 등)를 직접 적지 않는다 — 그런 러너는 이를 감싸는 스킬을 두고 그 스킬명을 적는다. (미채택)이면 수동 절차만 돈다. -->

- **소환할 스킬**: `(예: mino-qa / (미채택))`
- **입력 시나리오**: `(예: /plan/pr{N}/consumable/user-test-cases.md)`
- **판정 리포트 경로**: `(예: /plan/pr{N}/consumable/qa-report.md)`
- **미가용 시 동작**: `(예: 시뮬레이터·CLI 미가용이면 전 항목을 사용자 수동 확인 대상으로 넘긴다)`
...
<!-- 채우는 법: 프로젝트 컨벤션 외에 구현자·리뷰어가 1차 소스로 읽을 스킬을 적는다. (없음)이면 프로젝트 컨벤션만 적용된다. -->

- **스킬**: `(예: swiftui-expert, swift-concurrency, swift-testing-expert / (없음))`
- **적용 지점**: `(예: 구현·리뷰 양쪽 / 리뷰만)`
